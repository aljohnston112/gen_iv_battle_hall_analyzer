#include "battle_hall_analyzer.h"
    ...
    for (uint8_t group_number = 1; group_number <= 4; group_number++) {
        ...
        for (const uint8_t rank : GROUP_TO_RANKS.at(group_number)) {
            ...
            for (int8_t over_2 = 0; over_2 < NUMBER_OF_TYPES; over_2++) {
                ...
                for (auto& opponent_pokemon : hall_pokemon) {
...
struct BattleEntryEq {
    bool operator()(const BattleHallEntry& a, const BattleHallEntry& b) const {
...
struct ResultEntry {
    ...
    std::vector<const MoveInfo*> opponent_moves;
};
        ...
        for (uint8_t rank = 1; rank <= 10; rank++) {
            ...
            for (uint8_t over_2 = 0; over_2 < NUMBER_OF_TYPES; over_2++) {
                auto& hall_pokemon =
                    over_2_to_hall_pokemon.at(over_2);
                for (auto& opponent_pokemon : hall_pokemon) {
                    for (auto& player_pokemon_nature_variants :
                         player_pokemon_forms | std::views::values
                    ) {
                        for (auto& player_pokemon :
...
void do_battle(
    const BattleHallEntry& battle_entry,
    ...
    auto moves = std::vector<const MoveInfo*>{};
    for (const auto& [move, _, _] : player_moves) {
        moves.emplace_back(&MOVE_INFO_MAP.at(static_cast<int>(move)));
    }
    auto o_moves = std::vector<const MoveInfo*>{};
    for (const auto& [move, _, _] : opponent_moves) {
        o_moves.emplace_back(&MOVE_INFO_MAP.at(static_cast<int>(move)));
    }
    promise.set_value(
        ResultEntry{
            ...
            .opponent_moves = std::move(o_moves)
        }
    );
}
...
> simulate_battles(
    ...
    std::unordered_map<
        ...
        std::unordered_map<const MoveInfo*, int>
    > used_moves{};
    for (auto& [form, battles] :
         form_to_battles
    ) {
        const auto battle_result_entries =
            thread_pool::ThreadPool::getCPUWorkInstance()->
            createAndRunTasks<ResultEntry, std::vector<BattleHallEntry>,
                              BattleHallEntry>(
                do_battle,
                battles
            );
...
void print_walls(
    ...
    for (const auto& [
        ...
        ][rank - 1][over_2].emplace_back(opponent, player_moves, opponent_moves);
    }
