...
public:
    ...
    std::pair<bool, std::vector<const MoveInfo*>> battle() {
        ...
        if (player_goes_first) {
            check_drizzle(*this, player_state);
            check_drought(*this, player_state);
            check_sandstorm(*this, player_state);
            check_snow_warning(*this, player_state);

            check_drizzle(*this, opponent_state);
            check_drought(*this, opponent_state);
            check_sandstorm(*this, opponent_state);
            check_snow_warning(*this, opponent_state);
        } else {
            check_drizzle(*this, opponent_state);
            check_drought(*this, opponent_state);
            check_sandstorm(*this, opponent_state);
...
private:
    static void apply_end_of_turn_effects(
    ...
    static void check_mold_breaker(
    ...
    static void check_snow_warning(
        BattleState& battle_state,
        const PokemonState& pokemon_state
    ) {
...
BestMove BattleState::choose_move_against_defender(
    ...
    if (attacker_is_player) {
        // Since attacker's move is calculated first,
        // we should predict what the defender's next move is
    ...
    for (const auto& attacker_move :
        ...
        const int8_t attacker_move_priority = get_move_priority(attacker_move);
        const int8_t defender_move_priority =
            get_move_priority(defender_chosen_move.move);
    ...
    const auto defender_ability = defender_state.get_ability();
    const auto defenders_last_used_move = defender_state.get_last_used_move();
    for (const auto& move : attacker_pokemon.moves) {
        if (move_has_flag(
                move->move,
            ...
            if (move->move == Move::Absorb ||
                ...
                if (attacker_health < attacker_state.max_health / 2 &&
                    defenders_last_used_move.damage < attacker_state.max_health
                    / 2
                ) {
                    best_move.move = move;
                    best_move.damage = damage;
                ...
                if (move->move == Move::SunnyDay &&
                    ...
                    if (defender_chosen_move.damage > 0) {
                        const auto hits_taken =
                            attacker_health / defender_chosen_move.damage;
                        ...
                        if (hits_taken > 3 &&
                            defender_chosen_move.move->type ==
                            PokemonType::WATER
                        ) {
                            best_move.move = move;
                            best_move.damage = 0;
                    ...
                    if (defender_chosen_move.damage > 0) {
                        ...
                        if (defender_chosen_move.move->move !=
                            Move::TripleKick
                        ) {
                            damage_to_attacker =
                            ...
                            for (int i = 0; i < 3; i++) {
                                MoveInfo temp = *defender_chosen_move.move;
                                temp.power = (i + 1) * 10;
                                damage_to_attacker +=
                                    defender_state.get_damage_of_attacker_move(
                                        &temp,
                                        defender_state,
                                        weather,
                                        is_mid_turn
    ...
    if (CHOICE_ITEMS.contains(attacker_item) &&
        !is_choiced &&
        best_move.move != nullptr
    ) {
            ...
            for (int i = 0; i < attacker_chosen_move.times_to_hit; i++) {
                ...
                if (move_has_flag(
                    ...
                    if (attacker_move == Move::TripleKick &&
                        ...
                        const auto backup = attacker_chosen_move.move;
