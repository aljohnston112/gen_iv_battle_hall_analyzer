...
#define hungarian_test_alloc(X) do { \
		if ((void *)(X) == NULL){ \
		    fprintf( \
		        stderr, \
		        "Out of memory in %s, (%s, line %d).\n", \
		        __FUNCTION__, \
		        __FILE__, \
		        __LINE__ \
		    ); \
...
void hungarian_print_matrix(int** C, const int rows, const int cols) {
    fprintf(stderr, "\n");
...
int hungarian_imax(const int a, const int b) {
    return (a < b) ? b : a;
}
...
int hungarian_init(
    ...
    int max_cost = 0;
    const int org_cols = cols;
    const int org_rows = rows;
    ...
    if (mode == HUNGARIAN_MODE_MAXIMIZE_UTIL) {
        ...
        fprintf(
            stderr,
            "%s: unknown mode. "
            "Mode was set to HUNGARIAN_MODE_MINIMIZE_COST !\n",
            __FUNCTION__
        );
...
void hungarian_solve(hungarian_problem_t* p) {
    int i, j, k, l, s, q, unmatched;

    int cost = 0;
    int m = p->num_rows;
    int n = p->num_cols;

    int* col_mate = calloc(p->num_rows, sizeof(int));
    hungarian_test_alloc(col_mate);

    int* unchosen_row = calloc(p->num_rows, sizeof(int));
    hungarian_test_alloc(unchosen_row);

    int* row_dec = calloc(p->num_rows, sizeof(int));
    hungarian_test_alloc(row_dec);

    int* slack_row = calloc(p->num_rows, sizeof(int));
    hungarian_test_alloc(slack_row);

    int* row_mate = calloc(p->num_cols, sizeof(int));
    hungarian_test_alloc(row_mate);

    int* parent_row = calloc(p->num_cols, sizeof(int));
    hungarian_test_alloc(parent_row);

    int* col_inc = calloc(p->num_cols, sizeof(int));
    hungarian_test_alloc(col_inc);

    int* slack = calloc(p->num_cols, sizeof(int));
